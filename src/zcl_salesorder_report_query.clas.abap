CLASS zcl_salesorder_report_query DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_salesorder_report_query IMPLEMENTATION.
  METHOD if_rap_query_provider~select.
    DATA: lt_result TYPE TABLE OF zi_salesorder_report.
    ...
    " Logic tùy chỉnh: Tính toán category
    LOOP AT lt_data INTO DATA(ls_data).
      DATA(ls_result) = VALUE zi_salesorder_report(
        ...
        category        = COND #( WHEN ls_data-totalamount > 2000
                                  THEN 'PREMIUM'
                                  WHEN ls_data-totalamount > 1000
                                  THEN 'STANDARD'
                                  ELSE 'BASIC' )
      ).
